CREATE FUNCTION [dbo].[GetFilteredProductListItems]
(
	@categoryId int
)
...
BEGIN
	...
	SELECT	p.Id, 
		...
		FROM dbo.Product p with(nolock)
			...
			INNER JOIN dbo.ProductCategory pc with(nolock) on pc.ProductId = p.Id
		WHERE ( @categoryId IS NULL OR pc.CategoryId = @categoryId )
